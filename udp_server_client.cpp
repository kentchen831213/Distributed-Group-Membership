...
string false_logFileName="false_log_fp"+to_string(100)+"vm_packet-loss-rate"+to_string(0.9)+"_self"+to_string(100)+
                   "_trial"+to_string(100)+".txt";
//ifstream infile;
...
void send_status(string* input_command,int self_VM_number,vector<vector<string>>*  maintain_list, vector<string>* ip_list) {
    while(1){auto start_time_point = high_resolution_clock::now();
        ...
        string update_other_vm_status = create_udp_client_socket((*ip_list)[INTRODUCER_NUMBER-1].data(),self_VM_number,whole_command, false,1,maintain_list);
        ...
        auto end_time_point = high_resolution_clock::now();
        auto duration = duration_cast<microseconds>(start_time_point  - end_time_point);
        auto remain_time=FAIL_PERIOD*REJOIN_PERIOD_FACTOR*1000000-duration.count();
        if(remain_time>0){
            usleep(remain_time);
        }
        if(toLeave){break;}
...
// "172.22.158.200","172.22.94.200","172.22.156.201","172.22.158.201","172.22.94.201"};
    ...
    //send status to introducer
    thread send_status_to_introducer_thread;
    send_status_to_introducer_thread=std::thread(send_status,&input_command,self_VM_number,&maintain_list,&ip_list);
    //send_status(&input_command,self_VM_number,&maintain_list,&ip_list);
    ...
    for(int i =0; i<VM_COUNT;i++){
        //mtx.lock();
        int vm_number = (self_VM_number+i)%total_vm_num;
    ...
    for(int i =0; i<VM_COUNT;i++){
        int vm_number = (self_VM_number-i+total_vm_num)%total_vm_num;
        string ip_address=ip_list[vm_number];
    ...
    send_status_to_introducer_thread.join();
    return 0;
}
